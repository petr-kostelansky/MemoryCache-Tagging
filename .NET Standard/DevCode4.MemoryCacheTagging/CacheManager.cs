...
namespace DevCode4.MemoryCacheTagging
{
    public class CacheManager : ICacheManager
    {
        ...
        {
            cached = false;
            cache.TryGetValue(key, out CacheItemWrapper cacheItemWrapper);
        ...
        public void RemoveByTag<TEntityEnum>(TEntityEnum cacheEntity, int entityId)
        {
        ...
        private void Add(string key, CacheItemWrapper value, double duration)
        {
            ...
            option.RegisterPostEvictionCallback(CacheItemRemovedEvent);
        ...
        private void CacheItemRemovedEvent(object key, object value, EvictionReason reason, object state)
        {
            if (reason == EvictionReason.Expired
                || reason == EvictionReason.Removed)
            {
                var itemWrapper = value as CacheItemWrapper;
